...
def _read_default_config(filename: str) -> Optional[str]:
    """Return the textual content of a config resource shipped with the package."""

    try:
        resource = resources.files("assistant_demo.configs").joinpath(filename)
...
def _load_api_key(config_path: Optional[Path]) -> str:
    ...
    if config_path is not None and config_path.exists():
        ...
        raw_config = _read_default_config("embedding.json")

    if raw_config:
        try:
            payload = json.loads(raw_config)
        except json.JSONDecodeError as exc:  # pragma: no cover - defensive
            raise EmbeddingClientError("Invalid JSON in embedding config") from exc
...
class EmbeddingClient:
    ...
    def __post_init__(self) -> None:
        if not self.api_key:
            self.api_key = _load_api_key(self.config_path)
        if not self.api_key:
